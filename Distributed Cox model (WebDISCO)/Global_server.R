...
if (!file.exists("Global_times_output.csv")) {
  ...
  if (file.exists((paste0("sumExp", K, "_output_", ite, ".csv")))){
    ...
    if (ite > 1){
      ...
      temp <- abs(old_beta)/sqrt(diag(fisher_info))
      p_vals <- 2*(1 - pnorm(temp$x))
      
      # Exporting final results
      output <- cbind(old_beta, exp(old_beta), exp(-old_beta),  exp(old_beta - se), exp(old_beta + se), sqrt(var), p_vals)
      output <- format(output, digits = 6, nsmall = 6)
      colnames(output) <- c(" coef", " exp(coef)", " exp(-coef)", " lower .95", " upper .95", " se(coef)", " Pr(>|z|)")
      rownames(output) <- paste0("X", seq_len(length(old_beta$x)))
      
      # Write the output to a CSV file
      write.csv(output, file = paste0("Results_iter_", ite-1, ".csv"), quote = FALSE, row.names = TRUE)
    }
