...
Dlist <- sort(unique(c(times1D, times2D, times3D))) # D inclus tous les temps ou juste les deces? Pour l'instant, tout
D <- length(Dlist)
...
beta <- c(0, 0)   # Cheap initialization, eventually calculate local betas to initialize (converge style en 3 itérations)
...
for (i in 1:8) {
  ...
  sumDi_Norm_ZrExp_Exp <- colSums(Norm_ZrExp_Exp[1:138, ])
  ...
  sumDi_Norm_Value11 <- -sum(Norm_value11[1:138])
  ...
  sumDi_Norm_Value12 <- -sum(Norm_value12[1:138])
  ...
  sumDi_Norm_Value22 <- -sum(Norm_value22[1:138])
